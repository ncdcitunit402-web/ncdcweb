<?php
session_start();
if(!isset($_SESSION['admin'])){
  header("Location: login.php");
  exit;
}
include("includes/session_check.php");
include("../config/db.php");
include_once __DIR__.'/includes/header.php';

/* =======================
   DELETE VACANCY
======================= */
if (isset($_GET['delete_id'])) {
    $id = intval($_GET['delete_id']);

    $res = mysqli_query($conn, "SELECT pdf_file FROM vacancies WHERE id=$id");
    if ($row = mysqli_fetch_assoc($res)) {
        $filePath = "../uploads/vacancies/" . $row['pdf_file'];
        if (file_exists($filePath)) {
            unlink($filePath);
        }
    }

    mysqli_query($conn, "DELETE FROM vacancies WHERE id=$id");
    header("Location: ".$_SERVER['PHP_SELF']);
    exit;
}

/* =======================
   INSERT VACANCY
======================= */
if (isset($_POST['submit'])) {
$title = trim($_POST['title']);

if (!preg_match("/^[A-Za-z0-9 ]+$/", $title)) {
    echo "<script>
            alert('Only letters, numbers and spaces are allowed');
            history.back();
          </script>";
    exit;
}

$title = mysqli_real_escape_string($conn, $title);
    $title     = mysqli_real_escape_string($conn, $_POST['title']);
    $last_date = mysqli_real_escape_string($conn, $_POST['last_date']);

    // ===== PDF VALIDATION =====
$file_ext  = strtolower(pathinfo($_FILES['pdf']['name'], PATHINFO_EXTENSION));
$file_type = $_FILES['pdf']['type'];

// Allow only PDF
if ($file_ext !== 'pdf' || $file_type !== 'application/pdf') {
    echo "<script>alert('Only PDF files are allowed'); history.back();</script>";
    exit; // ⛔ stop upload
}

$pdf_name = time() . "_" . basename($_FILES['pdf']['name']);
$tmp_name = $_FILES['pdf']['tmp_name'];

$upload_path = "../uploads/vacancies/";

if (!is_dir($upload_path)) {
    mkdir($upload_path, 0777, true);
}

move_uploaded_file($tmp_name, $upload_path . $pdf_name);

    $sql = "INSERT INTO vacancies (title, last_date, pdf_file, status)
            VALUES ('$title', '$last_date', '$pdf_name', 1)";
    mysqli_query($conn, $sql);

    header("Location: ".$_SERVER['PHP_SELF']);
    exit;
}
?>

<!DOCTYPE html>
<html>
<head>
<title>Manage Vacancies</title>

<style>
body{
  font-family:"Segoe UI", Arial, sans-serif;
  background:#f4f6f9;
  padding:20px;
}
form{
  background:#fff;
  padding:20px;
  border-radius:8px;
  max-width:520px;
  box-shadow:0 2px 8px rgba(0,0,0,0.08);
}
input{
  width:100%;
  padding:8px;
  margin-bottom:15px;
}
button{
  background:#0b77b7;
  color:#fff;
  padding:10px 20px;
  border:none;
  border-radius:6px;
  cursor:pointer;
}
table{
  width:100%;
  margin-top:30px;
  border-collapse:collapse;
  background:#fff;
}
th, td{
  padding:10px;
  border:1px solid #ddd;
  text-align:left;
}
th{
  background:#f1f1f1;
}
/* BACK BUTTON */
.back-btn{
  display:inline-flex;
  align-items:center;
  gap:6px;
  margin-bottom:18px;
  padding:8px 14px;
  background:linear-gradient(135deg,#444,#222);
  color:#fff;
  text-decoration:none;
  border-radius:8px;
  font-size:14px;
  font-weight:600;
  transition:.3s;
}
.back-btn:hover{
  background:linear-gradient(135deg,#0b5ed7,#0d6efd);
  transform:translateX(-3px);
}
@media(max-width:600px){
  .back-btn{
    width:100%;
    justify-content:center;
  }
}
</style>
</head>

<body>
<a href="dashboard.php" class="back-btn">⬅ Back to Dashboard</a>
<h2>Add Vacancy</h2>

<form method="POST" enctype="multipart/form-data">
  <label>Vacancy Title</label>
  <input type="text" pattern="[A-Za-z0-9 ]+" 
       title="Only letters, numbers and spaces are allowed" name="title" required>

  <label>Last Date to Apply</label>
  <input type="date" name="last_date" required>

  <label>Upload PDF</label>
  <input type="file" name="pdf" accept=".pdf" required>

  <button type="submit" name="submit">Upload Vacancy</button>
</form>

<hr>

<h2>Uploaded Vacancies</h2>

<table>
<tr>
  <th>ID</th>
  <th>Title</th>
  <th>Last Date</th>
  <th>Actions</th>
</tr>

<?php
$result = mysqli_query($conn, "SELECT * FROM vacancies ORDER BY id DESC");
while ($row = mysqli_fetch_assoc($result)) {
?>
<tr>
  <td><?= $row['id']; ?></td>
  <td><?= $row['title']; ?></td>
  <td><?= date("d M Y", strtotime($row['last_date'])); ?></td>
  <td>
    <a href="../uploads/vacancies/<?= $row['pdf_file']; ?>" target="_blank">View PDF</a>
    |
    <a href="?delete_id=<?= $row['id']; ?>"
       onclick="return confirm('Delete this vacancy?');">Delete</a>
  </td>
</tr>
<?php } ?>

</table>

</body>
</html>
<?php include_once __DIR__.'/includes/footer.php'; ?>